...
func lossySliceRecon(y, cb, cr []uint8, width, height int) ([]byte, []uint8, []uint8, []uint8, error) {
	...
	for y0 := 0; y0 < height; y0 += 16 {
		for x0 := 0; x0 < width; x0 += 16 {
			mode := lossyLumaModeWithScratch(reconY, y, width, x0, y0, &modeScratch)
			qY := lossyBlockWithScratch(reconY, y, width, x0, y0, 16, mode, 0, &yScratch)
			qCb := lossyBlockWithScratch(reconCb, cb, width/2, x0/2, y0/2, 8, mode, 1, &cbScratch)
			qCr := lossyBlockWithScratch(reconCr, cr, width/2, x0/2, y0/2, 8, mode, 2, &crScratch)

			cand := lossyMPM(modes, width/16, x0/16, y0/16)
...
func lossyLumaModeWithScratch(recon, src []uint8, stride, x, y int, scratch *lossyBlockScratch) int {
	...
	for mode := intraPlanar; mode <= 34; mode++ {
		_, trial := lossyBlockDataWithScratch(recon, src, stride, x, y, 16, mode, 0, scratch)
		var dist int64
		for j := range 16 {
			for i := range 16 {
...
func lossyBlockWithScratch(recon, src []uint8, stride, x, y, n, mode, cIdx int,
	...
	coef, block := lossyBlockDataWithScratch(recon, src, stride, x, y, n, mode, cIdx, scratch)
	for j := range n {
...
func lossyBlockDataWithScratch(recon, src []uint8, stride, x, y, n, mode, cIdx int,
	...
	lossyPrediction(recon, stride, x, y, n, mode, cIdx, pred, avail, &scratch.ref)
	for j := range n {
		for i := range n {
...
func lossyPrediction(recon []uint8, stride, x, y, n, mode, cIdx int, out []uint8, avail []bool,
	ref *refSamples,
) {
	ref.n = n
	clear(avail)
...
func lossyMPM(modes []int, blocksWide, x, y int) [3]int {
